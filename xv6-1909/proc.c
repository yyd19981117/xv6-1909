...
found:
  ...
  //store the create time for FCFS
  acquire(&tickslock);
  p->ctime = ticks;
  release(&tickslock);
  ...
  *(uint*)sp = (uint)trapret;
  ...
  p->isthread = 0;   //Shradha changes

  //memset(p->stack, 0, 4096);   //Shradha changes
...
fork(void)
{
  ...
  np->parent = proc;
  *np->tf = *proc->tf;
  np->stride=proc->stride+10;
  np->pass=proc->pass;
  np->n_schedule=proc->n_schedule;
...
scheduler(void)
{
	...
	for(;;){
		...
		for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){		
			...
			else if ( p->ctime < firstp->ctime ) { firstp = p; }
		}
		if(firstp == 0) { goto retry; }
		p = firstp;
...
{
  acquire(&ptable.lock);
  cprintf("Pass:%d Stride:%d N_schedule:%d\n", proc->pass, proc->stride, proc->n_schedule);
  release(&ptable.lock);
  return 26;
}
